...
def dis_to_meshs(pts, pts_index, vectices, x_, y_, z_ ):
    ...
    if len(pts_index)!=0:
        distances[-1].data[0] = torch.max(distances[0:-1]).data[0] * 10.0 
    return distances
...
def dis_to_mesh(pts, pts_index, vertices, faces, x_, y_, z_):
    ...
    return torch.mean(dis_all_faces)
...
def pts_in_cell(pts, cell):
    ...
    if pts.is_cuda:
        cell = cell.cuda()
    inds = [i for i in range(N) if pts[0,i,0].data[0]>cell[0] and pts[0,i,0].data[0] < cell[3] 
                               and pts[0,i,1].data[0]>cell[1] and pts[0,i,1].data[0] < cell[4] 
                               and pts[0,i,2].data[0]>cell[2] and pts[0,i,2].data[0] < cell[5]] 
    return inds 
...
def offset_to_vertices(offset, x, y, z):
    ...
    if offset.is_cuda:
        ...
        dtype = torch.FloatTensor
    one = Variable(torch.ones(1).type(dtype), requires_grad=True)
    p = [ [0.5-offset[0, x+1, y+1, z  ],    1.0*one,    0.0*one], #0
          [1.0*one,     0.5-offset[1, x+1, y+1, z ],    0.0*one], #1
          [0.5-offset[0, x+1, y  , z  ],    0.0*one,    0.0*one], #2
          [0.0*one,     0.5-offset[1, x  , y+1, z ],    0.0*one], #3

          [0.5-offset[0, x+1, y+1, z+1],    1.0*one,    1.0*one], #4
          [1.0*one,     0.5-offset[1, x+1, y+1,z+1],    1.0*one], #5
          [0.5-offset[0, x+1, y  , z+1],    0.0*one,    1.0*one], #6
          [0.0*one,     0.5-offset[1, x  , y+1,z+1],    1.0*one], #7

          [0.0*one,     1.0*one,     0.5-offset[2, x  ,y+1,z+1]], #8
          [1.0*one,     1.0*one,     0.5-offset[2, x+1,y+1,z+1]], #9
          [1.0*one,     0.0*one,     0.5-offset[2, x+1,y  ,z+1]], #10
          [0.0*one,     0.0*one,     0.5-offset[2, x  ,y  ,z+1]]] #11
    return p 
